...
def determine_fftype(ffname, verbose=False):
    ...
    if len(fsplit) == 2:
        ...
        if verbose:
            logger.info("Guessing from extension (you may specify type with filename:type) ...")
        ffname = fsplit[0]
...
class FF(forcebalance.BaseClass):
        ...
        for ln, line in enumerate(self.ffdata[ffname]):
            ...
            if rmark is not None:
                ...
                while parse < stopparse:
                    ...
                    self.assign_field(
                        prep, pid, ffname, ln, pfld, "MINUS_" in sline[parse + 1] and -1 or 1
                    )
                    parse += 2
    ...
    def addff_xml(self, ffname):
        ...
        if len(scriptElements) > 1:
            ...
            if os.path.exists(absScript):
                logger.error(
                    "XML file " + absScript + " already exists on disk! Please delete it\n"
                )
                raise RuntimeError
        ...
        for e in self.ffdata[ffname].getroot().findall(".//*[@parameterize]"):  # RVK WORKS
            ...
            for p in parameters_to_optimize:
                ...
                if p not in e.attrib:
                    ...
                    logger.error(
                        "Parameter '%s' is not found for '%s', please check %s"
                        % (p, e.attrib["type"], ffname)
                    )
                    raise RuntimeError
                element_tree = self.ffdata[ffname]  # RVK added this
                pid = self.Readers[ffname].build_pid(
                    e, p, element_tree
                )  # RVK edited corresponding function in smirnoffio.py
        ...
        for e in self.ffdata[ffname].getroot().findall(".//*[@parameter_repeat]"):
            ...
            for field in e.attrib["parameter_repeat"].split(","):
                ...
                if parameter_name not in e.attrib:
                    ...
                    logger.error(
                        "Parameter '%s' is not found for '%s', please check %s"
                        % (parameter_name, e.attrib["type"], ffname)
                    )
                    raise RuntimeError
                element_tree = self.ffdata[ffname]  # RVK added this
                dest = self.Readers[ffname].build_pid(
                    e, parameter_name, element_tree
                )  # RVK edited corresponding function in smirnoffio.py
                ...
                if src in self.map:
                    ...
                    warn_press_key(
                        "Warning: You wanted to copy parameter from %s to %s, but the source parameter does not seem to exist!"
                        % (src, dest)
                    )
    ...
    def make(self, vals=None, use_pvals=False, printdir=None, precision=12):
        ...
        xml_lines = OrderedDict(
            [(fnm, list(newffdata[fnm].iter())) for fnm in self.fnms if self.ffdata_isxml[fnm]]
        )
        ...
        for i in range(len(self.pfields)):
            ...
            # columns.
                ...
                newffdata[fnm][ln] = (
                    "".join(
                        [
                            (whites[j] if (len(whites[j]) > 0 or j == 0) else " ") + sline[j]
                ...
                for i, pName in enumerate(self.map):
                    ...
                    if fnm == f or fnm == ffnameScript:
                        if fnm.endswith("offxml"):
                            ttstr = "/".join(
                                [pName.split("/")[0], pName.split("/")[1], pName.split("/")[2]]
                            )
                        else:
                ...
                ListOfAtoms = list(
                    itertools.chain(
                        *[
                            [
                                e.get("type")
                                for e in self.ffdata[k].getroot().xpath("//Residue/Atom")
                            ]
